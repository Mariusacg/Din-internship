...
public class ChatWebSocketHandler extends TextWebSocketHandler {
    ...
    public void afterConnectionEstablished(WebSocketSession session) throws Exception {
        ...
        new Thread(new NewMessagesTask(session)).start();
    }
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        Loggers.infoLogger.info("Data received from Websocket: " + payload);
        Message msg = gson.fromJson(payload, Message.class);

        session.sendMessage(new TextMessage(gson.toJson(msg, Message.class)));
        ...
        if (Usefullstuff.getINSTANCE().getActiveGroup() == null) {
            Loggers.infoLogger.info(service.sendDataUDP(msg));
        } else {
